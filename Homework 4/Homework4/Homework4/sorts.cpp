...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Sensor*> auxSensorsPointers;
		for (int i = 0; i < auxSensors.size(); i++)
		{
			auxSensorsPointers.push_back(&(auxSensors[i]));
		}

		// TODO:  Sort the vector of pointers using the STL sort algorithm
		//        with compareSensorPtr as the ordering relationship.
		sort(auxSensorsPointers.begin(), auxSensorsPointers.end(), compareSensorPtr);
		...
		{
			swap(sensors[i], *auxSensorsPointers[i]);
		}
